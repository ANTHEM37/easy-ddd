...
一个面向 Java/Spring 的轻量级 DDD 基础设施与业务编排套件，内置
CQRS（命令/查询总线）、领域事件发布与处理、仓储抽象、异步执行器配置、以及可组合的业务流程编排（BizFlow）。
...
class RegisterUserHandler implements ICommandHandler<RegisterUserCommand, String> {
    @Override
    public String handle(RegisterUserCommand cmd) { /* 业务 */
        return "OK";
    }

    @Override
    public Class<RegisterUserCommand> getSupportedCommandType() {
...
class GetUserProfileHandler implements IQueryHandler<GetUserProfileQuery, UserProfile> {
    @Override
    public UserProfile handle(GetUserProfileQuery q) { /* 读取 */
        return new UserProfile();
    }

    @Override
    public Class<GetUserProfileQuery> getSupportedQueryType() {
...
class UserRegisteredHandler extends AbstractEventHandler<UserRegisteredEvent> {
    public Class<UserRegisteredEvent> getSupportedEventType() {
...
class UserAggregate extends AbstractAggregateRoot<String> { /* ... */
    void markRegistered() {
        addDomainEvent(new UserRegisteredEvent(getId()));
    }
}
```
...
- 分层清晰：Common 定义契约与编排，Domain 表达模型与事件，Application 编排用例并桥接总线，Infrastructure 提供与 Spring
  深度集成的实现。
...
- 处理器约定：实现 `ICommandHandler` 或 `IQueryHandler`，并实现 `getSupportedCommandType()` / `getSupportedQueryType()`
  返回支持的消息类型。
...
- 通用属性：`corePoolSizeMultiplier`、`maxPoolSizeMultiplier`、`queueCapacity`、`keepAliveSeconds`、
  `rejectedExecutionPolicy`（`CALLER_RUNS`/`DISCARD_OLDEST`/`DISCARD`/`ABORT`）、`waitForTasksToCompleteOnShutdown`、
...
## 发布到 Maven 中央仓库（Central Portal）

- 前置准备：
    - 迁移或创建命名空间至 Central Publisher Portal，并生成「User Token」。
    - 安装并生成 GPG 密钥，确保可签名（`gpg --full-generate-key`）。
    - 将公钥上传到公钥服务器，例如 `keyserver.ubuntu.com`。
- 配置 `~/.m2/settings.xml`（使用 `serverId: central`）：
